...
for item in Specimen_Array:
    if float(Blood_data[item[1]] > 0.0):
        difference[item[1]] = float(item[2]) / float(Blood_data[item[1]])
        ...
        if float(item[2]) == 0.0:
            if float(Blood_data[item[1]]) > float(Current_Smallest_One):
                Current_Smallest_Two = float(Current_Smallest_One)
                SmallestID_Two = SmallestID_One
                Current_Smallest_One = float(Blood_data[item[1]])
                SmallestID_One = item[1]
            elif float(Blood_data[item[1]]) > float(Current_Smallest_Two):
                Current_Smallest_Two = float(Blood_data[item[1]])
