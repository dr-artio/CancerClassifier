...
def grid_search_cv(samples, targets, data, k, tuned_params, svr, path):
    ...
    print len(data[0])
    pickle.dump(clf.best_estimator_, open(path, 'w+'))
...
if __name__=='__main__':
    ...
    print "Linear SVM..."
    clf = grid_search_cv(samples, target_indices, data, 5, parameters, svr, "SVMlin15.pickle")
    print "Validation..."
    ...
    print "Random Forest..."
    clf = grid_search_cv(samples, target_indices, data, 5, parameters, svr, "RandomForest15.pickle")
    print "Validation..."
    tr = train(clf, samples, target_indices, data, 5)
    print "Accuracy: %0.2f (+/- %0.2f)" % (tr[1], tr[2])
    ...
    svr = svm.SVC()
    print "SVM..."
    clf = grid_search_cv(samples, target_indices, data, 5, parameters, svr, "SVM15.pickle")
    print "Validation..."
    tr = train(clf, samples, target_indices, data, 5)
    print "Accuracy: %0.2f (+/- %0.2f)" % (tr[1], tr[2])
